...
def all_series_dict(all_files_list, args):
    ...
    node_names = [
        file.split("/")[-1].split(".")[0].split("-")[-1] for file in all_files_list
    ]
...
def main():
    ...
    if not os.path.exists(
        os.path.join(os.getcwd(), "log", "output_txts", args.scenario)
    ):
    ...
    output_file.write(f"Scenario: {args.scenario} - Simulation {args.chosen_simulation.split('-')[-1]}\n")
    # Getting data path
    filenames = glob.glob(
        os.path.join(
            os.getcwd(),
    ...
    trains, tests, histories, predictions, conf_intervals = create_trains_and_tests(
        all_files, features, size
    )
    list_communicating_nodes_from_train = extract_list_nodes(
        original_net_traffic, size, args
    )
    dict_nodes_dests_from_train = extract_nodes_dests(original_net_traffic, size, args)
    node_names = [
        file.split("/")[-1].split(".")[0].split("-")[-1] for file in all_files
    ]
    ...
    for time_step in range(test_length):
        ...
        time_seconds = (
            (time_step + train_length) * args.time_window
            + args.time_window
            + args.time_window
        )
        ...
        for feature in features:
            print(
                f"\rARIMA on time_step: {time_seconds}/{args.simulation_time} and feature: {feature}",
                end="\r",
            )
            ...
            for node_index in range(n_nodes):
                node_name = (
                    all_files[node_index].split("/")[-1].split(".")[0].split("-")[-1]
                )
                node_full_name = (
                    all_files[node_index].split("/")[-1].split("\\")[-1].split(".")[0]
                )
                ...
                try:
                    ...
                    output, conf_int = model.predict(
                        n_periods=1, return_conf_int=True, alpha=alpha
                    )
                except:
                    pass
                toc = tm.perf_counter()
        ...
        if bool_raise_anomaly:
            ...
            output_file.write(
                f"Anomaly raised at time {time_seconds}. Devices involved: {list_nodes_raising_anomaly}\n"
            )
...
def parallel_main():
    ...
    # Setting file name for output txt
    if not os.path.exists(
        os.path.join(os.getcwd(), "log", args.output_dir, args.scenario)
    ):
    ...
    for time_step in range(test_length):
        ...
        for feature in features:
            print(
                "\rARIMA on time_step: {time_seconds}/{args.simulation_time} and feature: {feature}",
                end="\r",
            )
            if time_step == test_length - 1:
                print()
    ...
    toc_main = tm.perf_counter()
    output_file.write(f"Finished successfully after: {toc_main - tic_main}")
    # Closes txt file
    output_file.close()
    # Plot prediction times using box plot
    print(f"Whole main took: {toc_main - tic_main}")
