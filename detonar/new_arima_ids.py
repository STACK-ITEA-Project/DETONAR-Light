...
def all_daos_dict(files, features, args):
    # Extract nodes names for dictionary of features series
    node_names = [file.split('/')[-1].split('_')[0] for file in files]
    # Build dictionary where first entry is considered feature and second entry is considered node to extract the feature series of that node
    daos_dict =  {node_name: {feature: [] for feature in features} for node_name in
                   node_names}
    # For each device (file) get all the featrue series and put it in the corresponding dictionary entry
    for file in files:
        node_name = file.split('/')[-1].split('_')[0]
        for feature in features:
            series = get_series(file, feature)
            daos_dict[node_name] = series
    return daos_dict

def all_dios_dict(files, features, args):
    # Extract nodes names for dictionary of features series
    node_names = [file.split('/')[-1].split('_')[0] for file in files]
    total_timesteps = int(args.simulation_time/args.time_window)
    # Build dictionary where first entry is the node, and
    dios_dict =  {node_name: {time_step: [] for time_step in range(total_timesteps)} for node_name in
                   node_names}
    # For each device (file) get all the featrue series and put it in the corresponding dictionary entry
    for file in files:
        node_name = file.split('/')[-1].split('_')[0]
        for time_step in range(total_timesteps):
            row = get_row_list(file, time_step)
            series = []
            for el in row:
                if el == 0:
                    break
                series.append(get_node_name_from_id(el))
...
def get_nodes_in_train(nodes_stats, node_names, train_size, args):
    list_nodes = []
    # Get all nodes that transmitted at least one packet during training
    for node_id in node_names:
        train = nodes_stats[node_id].head(train_size)
        if (train['# DIO txd'].sum() + train['# DIS txd'].sum() + train['# DAO txd'].sum() + train[
            '# APP txd'].sum() > 0):
...
def parallel_main():
    ...
    # Setting file name for output txt
    if not os.path.exists(os.path.join(os.getcwd(), 'log', args.output_dir, args.scenario, str(int(args.simulation_time)))):
        os.makedirs(os.path.join(os.getcwd(), 'log', args.output_dir, args.scenario, str(int(args.simulation_time))))
    output_filename = os.path.join(os.getcwd(), 'log', args.output_dir, args.scenario, str(int(args.simulation_time)),
                                   args.chosen_simulation.split('-')[-1] + '.txt')
    output_file = open(output_filename, "w")
    output_file.write("Scenario: {} - Simulation {}\n".format(args.scenario, args.chosen_simulation.split('-')[-1]))
    # Getting data path
    ...
    dao_files = [item for item in all_files if 'dao' in item]
    DAO_features = ['TIME','PACKET_TYPE','CONTROL_PACKET_TYPE/APP_NAME','SOURCE_ID','DESTINATION_ID','PARENT_ID','SOURCE_IP','DESTINATION_IP','PARENT_IP']
    daos_dict = all_daos_dict(dao_files, DAO_features, args)
    dao_file = [item for item in all_files if 'DAO' in item]
    #daos = all_daos_file(dao_file, DAO_features, args)
    ...
    # Extract lists containing series for each device
    trains, tests, histories, predictions, conf_intervals = create_trains_and_tests(all_files, features, size)
    list_communicating_nodes_from_train = get_nodes_in_train(nodes_stats, node_names, size, args)
    # list_communicating_nodes_from_train = extract_list_nodes(original_net_traffic, size, args)
    # dict_nodes_dests_from_train = extract_nodes_dests(original_net_traffic, size, args)
    node_names = [file.split('/')[-1].split('_')[0] for file in all_files]
    # List to be filled with time performances
    ...
    for time_step in range(test_length):
        ...
        if (bool_raise_anomaly):
            ...
            '''
            for feature in features:
                #neighborhoods_full_name_dict[feature], neighborhoods_dict[feature] = extract_non_anomalous_nodes(
                #    all_nodes, list_nodes_raising_anomaly_full_name[feature])
                neighborhoods_full_name_dict[feature], neighborhoods_dict[feature] = extract_family(
                    daos, list_nodes_raising_anomaly_full_name[feature], time_seconds, args)
                # Nodes to check are the ones that have raised anomalies and their neighbours
                nodes_to_check_dict[feature] = list_nodes_raising_anomaly[feature] + neighborhoods_dict[feature]
                nodes_to_check_full_name_dict[feature] = list_nodes_raising_anomaly_full_name[feature] + \
                                                         neighborhoods_full_name_dict[feature]
