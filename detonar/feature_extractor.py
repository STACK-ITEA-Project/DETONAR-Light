...
def get_features(data, node_name, args):
    ...
    transmitted_count = transmitted_packets[
    ...
    if "Control_Packet" in transmitted_app_count:
        features[7] = (
            transmitted_app_count.sum() - transmitted_app_count["Control_Packet"]
        )
    else:
        features[7] = transmitted_app_count.sum()
    # Get all packets received/transmitted by the node
    all_packets = data[
        (data["RECEIVER_ID"] == node_name) | (data["TRANSMITTER_ID"] == node_name)
    ]
    app_list = (
        all_packets["CONTROL_PACKET_TYPE/APP_NAME"].value_counts().index.to_list()
    )
    control_list = [
        "DIO",
        "DAO",
    ...
    successful = transmitted_packets[
        transmitted_packets["PACKET_STATUS"] == "Successful"
    ]
    ...
    transmitted_app_pcks = transmitted_packets[
        transmitted_packets["PACKET_TYPE"] == "Sensing"
    ]
    ...
    if n_incoming_app_pcks == n_outgoing_app_pcks:
        ...
        features[13] = n_outgoing_app_pcks / n_incoming_app_pcks
...
def main():
    ...
    for file_index in range(len(filenames)):
        ...
        for node_index in range(len(nodes_names)):
            ...
            end_index = math.floor(
                (args.simulation_time - args.time_start) / args.time_window
            )
            matrix = []
            ...
            for index in range(start_index, end_index):
                ...
                features = get_features(
                    time_sequence, nodes_names[node_index], args
                )  # time_sequence_from_0_to_index, nodes_names[node_index], args)
            ...
            ]  # '# neighbours', '# next-hop IPs', 'incoming_vs_outgoing']
    ...
    toc = time.time()
    print(f"Time taken without parallelization: {(toc - tic)}")
    ...
    for file_index in range(len(filenames)):
        ...
        for node_index in range(len(nodes_names)):
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                for features in executor.map(
                    lambda x, y, z: get_features(x, y, z),
                    windows,
        ...
        dao_packets = dao_packets[
            [
                "SOURCE_ID",
                "DESTINATION_ID",
                "RECEIVER_ID",
                "TRANSMITTER_ID",
                args.time_feat_micro,
            ]
        ]
        dao_packets = dao_packets.rename(
            columns={args.time_feat_micro: args.time_feat_sec}
        )
        name = (
            args.out_feat_files
            + args.scenario
        ...
        dio_packets = dio_packets[
            ["TRANSMITTER_ID", "RECEIVER_ID", args.time_feat_micro]
        ]
        dio_packets = dio_packets.rename(
            columns={args.time_feat_micro: args.time_feat_sec}
        )
        name = (
            args.out_feat_files
        ...
        app_packets = app_packets.rename(
            columns={args.time_feat_micro: args.time_feat_sec}
        )
        name = (
            args.out_feat_files
        ...
        all_packets = all_packets.rename(
            columns={args.time_feat_micro: args.time_feat_sec}
        )
        name = (
            args.out_feat_files
