...
def extract_feature_before_and_after(features_series, nodes, time_step, args):
    ...
    for node in nodes:
        # Build dictionary containing boolean variables depending on the change in each feature
        attack_class_dict = {
...
def compare_rank(rank_one, rank_two):
    """
    Compares rank in regard to hops from root according to MRHOF objective function
    :param rank_one: A RPL Rank value
    :param rank_two: A RPL rank value
    :return: 0, -1 or 1
    returns 0 if ranks are equivalent in regard to hops from root
    returns -1 if rank_one is smaller than rank_two in regard to hops from root
    returns 1 if rank_one is greater than rank_two in regard to hops from root
    """
...
def check_ranks_br(
    features_series, nodes_and_features_dict, time_step, anomalous_nodes
):
...
def check_version_change(
    features_series, time_step, anomalous_nodes, nodes_and_features_dict, args
):
    ...
    for node in anomalous_nodes:
        ...
        if version_change_time > 0 and actual_version != previous_version:
...
def check_destination_change(
    features_series, time_step, anomalous_nodes, nodes_and_features_dict
):
    ...
    if change_in_destination:
        print(f"\tNODES CHANGING DESTINATION: {nodes_changing_destination}")
    return nodes_and_features_dict, change_in_destination, nodes_changing_destination


def find_attacker_ranks(feature_series, time_step, all_nodes):
    ...
    if attacker_node != []:
        print(
            f"Attacker node is {attacker_node}. It changed "
            f"rank at {nodes_and_times_dict[attacker_node[0]]}"
        )
    return attacker_node


def find_attacker_versions(feature_series, time_step, all_nodes):
    # Create dictionary with each node and corresponding time of rank change
    nodes_and_times_dict = {node_name: math.inf for node_name in all_nodes}
...
def find_attacker_ranks_and_versions(feature_series, time_step, all_nodes):
...
def classify_attack_from_dodag(
    ...
    if "SINKNODE-1" not in list_nodes_train:
        nodes_and_features_dict["SINKNODE-1"] = {
            feature: False for feature in all_features
        }
    # Check if list of communicating nodes is equal to the list obtained from training
    # Removed as it doesn't work with BR approach and Blackhole attack
    # anomalous_nodes, nodes_and_features_dict, change_in_communicating_nodes = \
    #    check_nodes_communicating(features_series, time_step, list_nodes_train,
    #    anomalous_nodes, nodes_and_features_dict, output_file, args)
    # Check if rank changed or not
    nodes_and_features_dict, change_in_ranks = check_ranks_br(
        features_series, nodes_and_features_dict, time_step, anomalous_nodes
    )
    # Check number of next hops
    nodes_and_features_dict, change_in_nexthops = check_nexthops(
        features_series, time_step, anomalous_nodes, nodes_and_features_dict, args
    )
    # Check versions
    nodes_and_features_dict, change_in_versions = check_version_change(
        features_series, time_step, anomalous_nodes, nodes_and_features_dict, args
    )
    # Check destinations and DAOs
    (
        nodes_and_features_dict,
        change_in_destination,
        nodes_changing_destination,
    ...
    if dodag_changed or change_in_versions or change_in_ranks or change_in_nexthops:
        if change_in_ranks and change_in_versions:
            attacker_node, attack_type = find_attacker_ranks_and_versions(
                features_series, time_step, list_nodes_train
            )
            ...
            output_file.write(f"\tVERSION ATTACK -> ATTACKER NODE {attacker_node}\n")
        ...
        if blackhole_attackers != []:
            output_file.write(
                f"\tBLACKHOLE/SEL FORWARD ATTACK -> ATTACKER NODE {blackhole_attackers}\n"
            )
        if hello_flood_attackers != []:
            output_file.write(
                f"\tHELLO FLOOD ATTACK -> ATTACKER NODE {hello_flood_attackers}\n"
            )
        if dis_attackers != []:
            output_file.write(f"\tDIS ATTACK -> ATTACKER NODE {dis_attackers}\n")
        if (
            blackhole_attackers == []
            and hello_flood_attackers == []
            and dis_attackers == []
        ):
            output_file.write("\tFALSE ALARM\n")
...
def extract_non_anomalous_nodes(all_nodes, nodes_raising_anomaly_full_name):
    ...
    neighborhood = [
        ...
        if neighbor not in nodes_raising_anomaly_full_name
    ]
    neighborhood_short = [neighbor.split("-")[-1] for neighbor in neighborhood]
    # Get unique elements
    neighborhood = list(set(neighborhood))
