...
def check_feature_with_arima(train, real_value, args):
    ...
    try:
        ...
        output, conf_int = model.predict(
            n_periods=1, return_conf_int=True, alpha=args.alpha
        )
    except:
        pass
...
def extract_feature_before_and_after(features_series, nodes, time_step, args):
    ...
    for node in nodes:
        # Build dictionary that will contain boolean variables depending on change in each feature
        attack_class_dict = {
            feature: False for feature in args.attack_classification_features
        }
        ...
        for feature in args.attack_classification_features:
            ...
            if feature == "# APP txd":
                ...
                attack_class_dict[feature] = check_feature_with_arima(
                    train, ground_truth, args
                )
            if feature == "# DIO txd":
            ...
            if feature == "# DIS txd":
                ...
                ground_truth = feature_s[time_step]
                attack_class_dict[feature] = check_feature_single_val(
                    previous_value, ground_truth
                )
        if attack_class_dict["# DIS txd"]:
            print(f"\tDevice {node} -> DIS ATTACK!!!!!")
        elif attack_class_dict["# DIO txd"]:
            print(f"\tDevice {node} -> HELLO FLOODING ATTACK!!!!")
        else:
            print(f"\tDevice {node} -> False alarm")


def check_communicating_nodes(
    net_traffic,
    time_step,
    list_nodes_train,
    anomalous_nodes,
    ...
    condition = (
        net_traffic[args.time_feat_micro] > (time_step + 1) * args.time_window * 1e6
    ...
    list_nodes = [
        ...
        if "SENSOR" in node or "SINKNODE" in node
    ]
    list_nodes.sort()
...
def multiple_check_communicating_nodes(
    ...
    if not list_nodes == list_nodes_train:
        ...
        for node in nodes_missing:
            nodes_and_features_dict[node]["# sensors"] = True
            print(f"\tDevice {node} -> CLONE-ID or SYBIL ATTACK!!!!")
            output_file.write(f"\tCLONE-ID or SYBIL ATTACK -> ATTACKER NODE: {node}\n")
            if node in anomalous_nodes:
                anomalous_nodes.remove(node)
    return anomalous_nodes, nodes_and_features_dict, change_in_communicating_nodes


def get_ranks_in_window(
    control_traffic, time_step, anomalous_nodes, args, is_actual_ranks
):
    # From original traffic get considered window
    condition = (
        control_traffic[args.time_feat_sec] > (time_step + 1) * args.time_window
    ...
    if len(data) != 0:
        ...
        for node_name in anomalous_nodes:
            ranks = (
                data[data["RECEIVER_ID"] == node_name]["RPL_RANK"]
                .value_counts()
                .index.to_list()
            )
            counter = 0
            ...
            while len(ranks) == 0 and not is_actual_ranks:
                counter += 1
                condition = (
                    control_traffic[args.time_feat_sec]
                    > (time_step + 1 - counter) * args.time_window
                ) & (
                    control_traffic[args.time_feat_sec]
                    < (time_step + 2 - counter) * args.time_window
                )
                data = control_traffic[condition]
                names = data["RECEIVER_ID"].str.split("-", n=1, expand=True)
                data.drop(columns=["RECEIVER_ID"], inplace=True)
...
def check_ranks_changed(
    previous_ranks, actual_ranks, nodes_and_features_dict, anomalous_nodes
):
    ...
    for node in anomalous_nodes:
        if node != "1":
            ...
            if len(ac_ranks) != 0:
                ...
                # Below comparisons are not used as of now,
                # but could be used to distinguish between different rank attacks
                if len(ac_ranks) > 1 and compare_rank(ac_ranks[-1], ac_ranks[0]) == 1:
                    nodes_and_features_dict[node]["greater rank"] = True
                elif (
                    len(ac_ranks) > 1 and compare_rank(ac_ranks[-1], ac_ranks[0]) == -1
                ):
                    nodes_and_features_dict[node]["smaller rank"] = True
                elif (
                    len(ac_ranks) == 1
                    and compare_rank(ac_ranks[0], prev_ranks[-1]) == 1
                ):
                    nodes_and_features_dict[node]["greater rank"] = True
                elif (
                    len(ac_ranks) > 1
...
def check_n_nexthops(
    net_traffic, time_step, anomalous_nodes, nodes_and_features_dict, args
):
    ...
    if len(data_after) != 0:
        ...
        # Check each anomalous node if it has gained a next hop IP address
        # (changing parent or destination)
        for node in anomalous_nodes:
            ...
            next_hop_ips = (
                all_transmitted_packets[
                    all_transmitted_packets["NEXT_HOP_IP"] != "ff02::1a"
                ]["NEXT_HOP_IP"]
                .value_counts()
                .index.to_list()
            )
...
def check_n_neighbors(
    ...
    for node in anomalous_nodes:
        # Get number of next hops before anomaly
        all_transmitted_packets = data_before[data_before["TRANSMITTER_ID"] == node]
        transmitted_dios = all_transmitted_packets[
            all_transmitted_packets["CONTROL_PACKET_TYPE/APP_NAME"] == "DIO"
        ]
...
def check_versions(
    ...
    while not data_found:
        condition = (
            net_traffic[args.time_feat_sec] > (time_step) * args.time_window - counter
        ) & (
            net_traffic[args.time_feat_sec]
            < (time_step + 1) * args.time_window - counter
        )
        data_before = net_traffic[condition]
    ...
    if len(data_after) != 0:
        ...
        for node in anomalous_nodes:
            ...
            versions_before = (
                transmitted_controls["RPL_VERSION"].value_counts().index.to_list()
            )
            # Get number of next hops after anomaly
            ...
            # If  there are more than one value for version present, note that version changed
            versions = versions_before + versions_after
...
def get_data_for_timewindow(net_traffic, time_step, args):
...
def find_attacker_ranks(net_traffic, time_step, all_nodes, args):
    found_rank_change_time = False
    # Get data after the anomaly, or further back if needed
    i = time_step
    while not found_rank_change_time:
        data_after = get_data_for_timewindow(net_traffic, i + 1, args)
        if not data_after.empty:
            ...
            for node in all_nodes:
                ...
                if not transmitted_dios.empty:
                    for it in range(len(transmitted_dios.index) - 1):
                        row_before = transmitted_dios.iloc[it]
                        row_after = transmitted_dios.iloc[it + 1]
                        rank_before = row_before["RPL_RANK"]
                        rank_after = row_after["RPL_RANK"]
                        if (
                            (rank_after != rank_before)
                            and not (math.isnan(rank_before))
                            and not (math.isnan(rank_after))
                        ):
                            change_time = row_after[args.time_feat_sec]
                            if change_time < first_change:
                                first_change = change_time
                                found_rank_change_time = True
    ...
    if attacker_node:
        print(f"Attacker node is {attacker_node[0]}."
              f" It first changed rank at {nodes_and_times_dict[attacker_node[0]]}")
    return attacker_node


def find_attacker_versions(net_traffic, time_step, all_nodes, args):
    found_version_change_time = False
    i = time_step
    # Get data after the anomaly
    while not found_version_change_time:
        data_after = get_data_for_timewindow(net_traffic, i + 1, args)
                ...
                if not transmitted_dios.empty:
                    for it in range(len(transmitted_dios.index) - 1):
                        ...
                        if (
                            ...
                            and not (math.isnan(version_before))
                            and not (math.isnan(version_after))
                        ):
                            change_time = row_after[args.time_feat_sec]
                            if change_time < first_change:
                                first_change = change_time
                                found_version_change_time = True
                nodes_and_times_dict[node] = first_change
        i -= 1
    # Check which node changed rank first
    min_change_time = math.inf
    ...
    if attacker_node != []:
        print(
            f"Attacker node is {attacker_node}. "
            f"It changed version at {nodes_and_times_dict[attacker_node]}"
        )
    attacker = []
...
def find_attacker_ranks_and_versions(net_traffic, time_step, all_nodes, args):
    ...
    if attacker_node_ranks != [] and attacker_node_versions == []:
        ...
        attack_type = "RANK"
        print(
            f"RANK ATTACK! -> Attacker node is {attacker_node}. "
            f"It changed rank at {nodes_and_ranks_times_dict[attacker_node]}"
        )
    if attacker_node_ranks == [] and attacker_node_versions != []:
        attacker_node = attacker_node_versions[:]
        attack_type = "VERSION"
        print(
            f"VERSION ATTACK! -> Attacker node is {attacker_node}. "
            f"It changed version at {nodes_and_versions_times_dict[attacker_node]}"
        )
    if attacker_node_ranks != [] and attacker_node_versions != []:
        if (attacker_node_versions[:] == "1") and (
            nodes_and_ranks_times_dict[attacker_node_ranks]
            < nodes_and_versions_times_dict[attacker_node_versions]
        ):
            attacker_node = attacker_node_ranks[:]
            attack_type = "RANK"
            print(
...
def classify_attack_from_dodag(
    features_series,
    all_packets,
    ranks_vers,
    anomalous_nodes,
    ...
    nodes_and_features_dict = {
        node_name: {feature: False for feature in all_features}
        for node_name in list_nodes_train
    }
    ...
    (
        anomalous_nodes,
        nodes_and_features_dict,
        change_in_communicating_nodes,
    ) = multiple_check_communicating_nodes(
        all_packets,
        time_step,
        list_nodes_train,
        anomalous_nodes,
    ...
    tic = tm.perf_counter()
    actual_ranks = get_ranks_in_window(
        ranks_vers, time_step, anomalous_nodes, args, True
    )
    if len(actual_ranks) > 0:
        data_found = False
        counter = 0
        while not data_found:
            previous_ranks = get_ranks_in_window(
                ranks_vers, time_step - counter - 1, anomalous_nodes, args, False
            )
            if len(previous_ranks) != 0:
                data_found = True
            counter += 1

        nodes_and_features_dict, change_in_ranks = check_ranks_changed(
            previous_ranks, actual_ranks, nodes_and_features_dict, anomalous_nodes
        )
    toc = tm.perf_counter()
    # Check number of next hops
    tic = tm.perf_counter()
    nodes_and_features_dict, change_in_nexthops = check_n_nexthops(
        all_packets, time_step, anomalous_nodes, nodes_and_features_dict, args
    )
    toc = tm.perf_counter()
    # Check versions
    tic = tm.perf_counter()
    nodes_and_features_dict, change_in_versions = check_versions(
        ranks_vers, time_step, anomalous_nodes, nodes_and_features_dict, args
    )
    toc = tm.perf_counter()
    ...
    for node in anomalous_nodes:
        ...
        for feature_class in args.attack_classification_features:
            ...
            if feature_class == "# APP txd" or feature_class == "incoming_vs_outgoing":
                train = feature_s[time_step - 30 : time_step]
                ground_truth = feature_s[time_step]
                nodes_and_features_dict[node][feature_class] = check_feature_with_arima(
                    train, ground_truth, args
                )
            if feature_class == "# DIO txd":
                train = feature_s[:time_step]
                ground_truth = feature_s[time_step]
                nodes_and_features_dict[node][feature_class] = check_feature_with_max(
                    train, ground_truth
                )
    ...
    if not change_in_communicating_nodes:
        if dodag_changed or change_in_versions or change_in_ranks or change_in_nexthops:
            if change_in_ranks and change_in_versions:
                attacker_node, attack_type = find_attacker_ranks_and_versions(
                    ranks_vers, time_step, list_nodes_train, args
                )
                print(f"\t{attack_type} ATTACK -> ATTACKER NODE {attacker_node}")
                output_file.write(
                    f"\t{attack_type} ATTACK -> ATTACKER NODE {attacker_node}\n"
                )
            elif change_in_ranks and not change_in_versions:
                attacker_node = find_attacker_ranks(
                    ranks_vers, time_step, list_nodes_train, args
                )
                print(f"\tRANKS ATTACK -> ATTACKER NODE {attacker_node}")
                output_file.write(f"\tRANKS ATTACK -> ATTACKER NODE {attacker_node}\n")
            elif change_in_versions and not change_in_ranks:
                attacker_node = find_attacker_versions(
                    ranks_vers, time_step, list_nodes_train, args
                )
                print(f"\tVERSION ATTACK -> ATTACKER NODE {attacker_node}")
                output_file.write(
                    f"\tVERSION ATTACK -> ATTACKER NODE {attacker_node}\n"
                )
            elif change_in_nexthops:
                attacker_node = find_attacker_worst_parent(
                    nodes_and_features_dict, list_nodes_train
                )
                print(f"\tWORST PARENT ATTACK -> ATTACKER NODE {attacker_node}")
                output_file.write(
                    f"\tWORST PARENT ATTACK -> ATTACKER NODE {attacker_node}\n"
                )
            else:
            ...
            for node in anomalous_nodes:
                if nodes_and_features_dict[node]["# APP txd"]:
                    if nodes_and_features_dict[node]["incoming_vs_outgoing"]:
                        print(f"\tDevice {node} -> BLACKHOLE/SEL FORWARD ATTACK!!!!!")
                        blackhole_attackers.append(node)
                    else:
                        print(f"\tDevice {node} -> False alarm")
                elif nodes_and_features_dict[node]["# DIO txd"]:
                    print(f"\tDevice {node} -> HELLO FLOOD ATTACK!!!!!")
            ...
            if (blackhole_attackers != []):
                output_file.write(f'\tBLACKHOLE/SEL FORWARD ATTACK -> ATTACKER NODE {blackhole_attackers}\n')
            if (hello_flood_attackers != []):
                output_file.write(f'\tHELLO FLOOD ATTACK -> ATTACKER NODE {hello_flood_attackers}\n')
            if (dis_attackers != []):
                output_file.write(f'\tDIS ATTACK -> ATTACKER NODE {dis_attackers}\n')
