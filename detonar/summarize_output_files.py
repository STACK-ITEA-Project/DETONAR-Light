...
def _process_file(result_file, full_results_file, expected_attacks_lines, scenario, args):
    ...
    nr_early_alarms = 0
    with open(result_file, "r", encoding="utf8") as scenario_result_file:
        lines = scenario_result_file.readlines()
        ...
        # Lists of times and attackers of the attacks found
        attack_dict = {
...
def main():
    ...
    #with open(output_filename, "w", encoding="utf8") as summary_file:
    with open(result_summary_filename, "w", encoding="utf8") as summary_file, open(
        ...
        for scenario_path in scenario_paths:
            info_paths.append(
                glob.glob(
                    os.path.join(
                        scenario_path, f"Packet_Trace_{args.simulation_time}s", "*.txt"
                    )
                )[0]
            )
        ...
        for scenario in scenarios:
            ...
            expected_attacks_lines = _get_expected_attacks_lines(info_file_path)
            ...
            if os.path.exists(
                ...
                for file in filenames:
                    (
                        ...
                        file, full_results_file, expected_attacks_lines, scenario, args
                    )
                    ...
                    if early_alarm_raised:
                        ...
                        total_early_alarms += nr_early_alarms
                summary_file.write(
                    f"Results: {correctly_classified_sims_result}/{total_sims} "
                    f"correctly classified,{correctly_identified_attackers_result}"
                    f"/{total_sims} identified attacker\n"
                )
                summary_file.write(
                    f"{alarms_raised}/{total_sims} alarms raised, "
                    f"{early_alarms_raised}/{total_sims} "
                    f"false positives before attack\n"
                )
                summary_file.write(
                    f"{total_early_alarms} early alarms were raised in total\n"
                )
                summary_file.write(
                    f"{sims_finished_successfully}/{total_sims} sims "
